...
export default function AdminTournamentsPage() {
    ...
    const del = async (id: string) => { if (!confirm('Delete?'))
        return; await supabase.from('tournaments').delete().eq('id', id); await loadData(); };
    ...
    const openEdit = async (t: any) => {
        ...
        if (linkedPost?.category)
            newsCategory = linkedPost.category;
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {tournaments.map(t => {
            ...
            return (<div key={t.id} style={{ background: '#071426', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
                {t.image_url && (<div style={{ height: 120, overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                      ...
                      {cats.map((cat: string) => (<span key={cat} style={{ ...F, fontSize: 9, letterSpacing: 1, padding: '2px 8px', borderRadius: 20, background: (CLASS_COLORS[cat] || '#6B7280') + '22', color: CLASS_COLORS[cat] || '#6B7280', border: `1px solid ${CLASS_COLORS[cat] || '#6B7280'}44` }}>{CLASS_ICONS[cat] ? CLASS_ICONS[cat] + ' ' : ''}{cat.toUpperCase()}</span>))}
                    </div>
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <div onClick={() => imageFileRef.current?.click()} style={{ border: '2px dashed rgba(255,255,255,0.12)', borderRadius: 10, padding: editing.image_url ? 10 : '28px 16px', textAlign: 'center', cursor: 'pointer', background: '#050505' }}>
                  {editing.image_url ? (<img src={editing.image_url} alt="Event preview" style={{ maxHeight: 140, maxWidth: '100%', borderRadius: 8, margin: '0 auto', display: 'block' }}/>) : (<>
                ...
                {editing.image_url && (<button type="button" onClick={() => setEditing({ ...editing, image_url: '' })} style={{ ...FB, fontSize: 11, color: '#DC2626', background: 'none', border: 'none', cursor: 'pointer', marginTop: 6, padding: 0 }}>Remove photo</button>)}
              </div>
              ...
              <div>
                ...
                return (<button key={cat} onClick={() => toggleCategory(cat)} style={{ ...F, fontWeight: 700, fontSize: 12, letterSpacing: 1, padding: '7px 14px', borderRadius: 20, border: `1.5px solid ${selected ? color : color + '44'}`, background: selected ? color + '22' : 'transparent', color: selected ? color : 'rgba(255,255,255,0.3)', cursor: 'pointer', transition: 'all 0.15s' }}>
                        {cat}
                      </button>);
            })}
                </div>
