...
export default function AdminGalleryPage() {
  ...
  const [items,setItems]=useState<any[]>(DEMO);const [editing,setEditing]=useState<any>(null);const [filter,setFilter]=useState('all');const [saving,setSaving]=useState(false);const [useDb,setUseDb]=useState(false);const [preview,setPreview]=useState('');const fileRef=useRef<HTMLInputElement>(null);

  useEffect(()=>{const ok=checkAuth();setAuthed(ok);setChecked(true);if(ok)fetchItems();},[]);
  const fetchItems=async()=>{const{data,error}=await supabase.from('gallery_items').select('*').order('created_at',{ascending:false});if(!error&&data){setItems(data);setUseDb(true);}};
  const handleFile=(e:React.ChangeEvent<HTMLInputElement>)=>{const f=e.target.files?.[0];if(!f||!editing)return;const r=new FileReader();r.onloadend=()=>{const u=r.result as string;setPreview(u);setEditing({...editing,image_url:u});};r.readAsDataURL(f);};
  const save=async()=>{
    if(!editing)return;setSaving(true);
    if(useDb){if(editing.id&&!DEMO.find(d=>d.id===editing.id))await supabase.from('gallery_items').update(editing).eq('id',editing.id);else await supabase.from('gallery_items').insert({...editing,created_at:new Date().toISOString()});await fetchItems();}
    else{if(editing.id)setItems(p=>p.map(x=>x.id===editing.id?editing:x));else setItems(p=>[{...editing,id:Date.now().toString()},...p]);}
    setEditing(null);setPreview('');setSaving(false);
  };
  const del=async(id:string)=>{if(!confirm('Delete?'))return;if(useDb){await supabase.from('gallery_items').delete().eq('id',id);await fetchItems();}else setItems(p=>p.filter(x=>x.id!==id));};
  const filtered=filter==='all'?items:items.filter(i=>i.category===filter);

  if(!checked)return null;
  if(!authed)return<LoginScreen title="Manage Gallery" onLogin={()=>{setAuthed(true);fetchItems();}}/>;
    ...
    <div style={{minHeight:'100vh',background:'#050505',color:'#F5F5F5'}}>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'24px 20px'}}>
        <div style={{display:'flex',gap:8,overflowX:'auto',marginBottom:20,paddingBottom:4}}>
          {['all',...CATS].map(c=>(
            <button key={c} onClick={()=>setFilter(c)} style={{...F,fontWeight:700,fontSize:11,letterSpacing:1,padding:'7px 14px',borderRadius:8,border:filter===c?'none':'1px solid rgba(255,255,255,0.08)',background:filter===c?'#DC2626':'#071426',color:filter===c?'#fff':'#B8C1CC',cursor:'pointer',whiteSpace:'nowrap' as const,flexShrink:0}}>{c.toUpperCase()}</button>
          ))}
        </div>
          ...
          {filtered.map(item=>(
            <div key={item.id} style={{background:'#071426',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,overflow:'hidden'}}>
              <div style={{height:150,background:'#050505',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',position:'relative'}}>
                {item.image_url?<img src={item.image_url} alt={item.title} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:36}}>🖼️</span>}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:'#071426',border:'1px solid rgba(255,255,255,0.1)',borderRadius:18,width:'100%',maxWidth:480,marginBottom:24}}>
            ...
            <div style={{padding:22,display:'flex',flexDirection:'column',gap:14}}>
              <div onClick={()=>fileRef.current?.click()} style={{height:150,background:'#050505',border:'2px dashed rgba(255,255,255,0.1)',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',overflow:'hidden'}}>
                {preview||editing.image_url?<img src={preview||editing.image_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{textAlign:'center'}}><div style={{fontSize:28,marginBottom:6}}>📷</div><div style={{...F,fontSize:13,color:'#B8C1CC'}}>TAP TO UPLOAD</div></div>}
              </div>
              ...
              <button onClick={save} disabled={saving} style={{background:'#DC2626',color:'#fff',border:'none',borderRadius:10,padding:'13px',...F,fontWeight:900,fontSize:17,letterSpacing:2,cursor:'pointer',opacity:saving?0.6:1}}>{saving?'SAVING...':'SAVE PHOTO'}</button>
            </div>
          </div>
        </div>
      )}
