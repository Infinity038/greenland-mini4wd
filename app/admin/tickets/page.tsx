...
export default function AdminTicketsPage() {
    ...
    async function loadData() {
        ...
        if (cfg)
            setEarlyBirdSlots(parseInt(cfg.value));
    }
    async function updateStatus(id: string) {
        ...
        if (status === 'payment_confirmed' && ticket?.payment_status !== 'payment_confirmed') {
            ...
            if (member) {
                ...
                if (grantFree)
                    setMsg(`✅ Updated · 🎁 ${hasRealName(ticket.member_name) ? ticket.member_name : ticket.member_email} earned a FREE ticket!`);
                else
                    setMsg('✅ Updated');
            }
            else {
            ...
            if (!ticket?.rewards_applied) {
                ...
                if (spend > 0 && ticket?.member_email) {
                    try {
                        ...
                        setMsg('⚠️ Updated, but rewards sync failed: ' + e.message);
                    }
                }
            }
        }
            ...
            try {
                ...
                setMsg('⚠️ Cancelled, but clawback failed: ' + e.message);
            }
            setMsg('✅ Updated');
        }
        else {
    ...
    return (<div style={s.page}>
      ...
      <div style={s.body}>
        ...
        ].map(s => (<div key={s.label} style={{ background: '#071426', border: `1px solid ${s.color}22`, borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
          {['all', 'awaiting_payment', 'proof_uploaded', 'payment_confirmed', 'cancelled'].map(f => (<button key={f} onClick={() => setFilter(f)} style={{ ...F, fontSize: 11, letterSpacing: 1, padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', background: filter === f ? '#DC2626' : 'rgba(255,255,255,0.06)', color: filter === f ? '#fff' : '#B8C1CC' }}>
              {f === 'all' ? 'ALL' : STATUS_LABELS[f]?.toUpperCase()}
            </button>))}
          <div style={{ width: 1, background: 'rgba(255,255,255,0.08)', margin: '0 4px' }}/>
          {['all', 'weekly_earlybird', 'weekly', 'season'].map(f => (<button key={f} onClick={() => setTypeFilter(f)} style={{ ...F, fontSize: 11, letterSpacing: 1, padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', background: typeFilter === f ? '#3B82F6' : 'rgba(255,255,255,0.06)', color: typeFilter === f ? '#fff' : '#B8C1CC' }}>
              {f === 'all' ? 'ALL TYPES' : TYPE_LABELS[f]?.toUpperCase()}
            </button>))}
        </div>

        {filtered.length === 0 ? (<div style={{ textAlign: 'center', padding: '60px 20px', ...FB, color: '#6B7280' }}>No tickets found.</div>) : filtered.map(ticket => {
            const nameIsReal = hasRealName(ticket.member_name);
            return (<div key={ticket.id} style={s.card}>
            ...
            {ticket.payment_proof_url && (<div style={{ marginBottom: 12, background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <select value={statuses[ticket.id] || ticket.payment_status} onChange={e => setStatuses(s => ({ ...s, [ticket.id]: e.target.value }))} style={{ ...s.inp, cursor: 'pointer' }}>
                {['awaiting_payment', 'proof_uploaded', 'payment_confirmed', 'cancelled'].map(st => (<option key={st} value={st}>{STATUS_LABELS[st]}</option>))}
              </select>
              <button onClick={() => updateStatus(ticket.id)} disabled={saving === ticket.id} style={{ background: '#22C55E', color: '#000', border: 'none', borderRadius: 8, padding: '8px 16px', ...F, fontWeight: 700, fontSize: 13, letterSpacing: 1, cursor: 'pointer', opacity: saving === ticket.id ? 0.5 : 1 }}>
                {saving === ticket.id ? '...' : 'SAVE'}
              </button>
              <button onClick={() => deleteTicket(ticket.id)} style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 8, padding: '8px 14px', ...F, fontWeight: 700, fontSize: 13, color: '#DC2626', cursor: 'pointer' }}>
                DELETE
              </button>
            </div>
          </div>);
