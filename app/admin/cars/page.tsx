...
export default function AdminCarsPage() {
    ...
    async function deleteCar(id: string) {
        if (!confirm('Delete this car registration?'))
            return;
    ...
    return (<div style={{ minHeight: '100vh', background: '#050505', color: '#F5F5F5' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
          {[{ label: 'Total', value: stats.total, color: '#3B82F6' }, { label: 'Pending', value: stats.pending, color: '#FACC15' }, { label: 'Approved', value: stats.approved, color: '#22C55E' }, { label: 'Rejected', value: stats.rejected, color: '#DC2626' }].map(s => (<div key={s.label} style={{ background: '#071426', border: `1px solid ${s.color}22`, borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
          {['all', 'pending', 'approved', 'rejected'].map(f => (<button key={f} onClick={() => setFilter(f)} style={{ ...F, fontSize: 11, letterSpacing: 1, padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', background: filter === f ? '#DC2626' : 'rgba(255,255,255,0.06)', color: filter === f ? '#fff' : '#B8C1CC' }}>
              {f.toUpperCase()}
            </button>))}
        </div>

        {/* Car list */}
        {filtered.length === 0 ? (<div style={{ textAlign: 'center', padding: 60, ...FB, color: '#6B7280' }}>No cars found.</div>) : filtered.map(car => (<div key={car.id} style={{ background: '#071426', border: `1px solid ${STATUS_COLORS[car.status] || '#333'}33`, borderRadius: 12, padding: 20, marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              {car.image_url ? (<img src={car.image_url} alt={car.name} style={{ width: 72, height: 72, objectFit: 'contain', borderRadius: 8, background: '#050505', border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}/>) : (<div style={{ width: 72, height: 72, borderRadius: 8, background: '#050505', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, flexShrink: 0 }}>🏎️</div>)}
              <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                {car.status !== 'approved' && (<button onClick={() => updateStatus(car.id, 'approved')} disabled={saving === car.id} style={{ background: '#22C55E', color: '#000', border: 'none', borderRadius: 6, padding: '8px 16px', ...F, fontWeight: 700, fontSize: 12, letterSpacing: 1, cursor: 'pointer' }}>
                    ✓ APPROVE
                  </button>)}
                {car.status !== 'rejected' && (<button onClick={() => updateStatus(car.id, 'rejected')} disabled={saving === car.id} style={{ background: 'rgba(220,38,38,0.1)', color: '#DC2626', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 6, padding: '8px 16px', ...F, fontWeight: 700, fontSize: 12, letterSpacing: 1, cursor: 'pointer' }}>
                    ✕ REJECT
                  </button>)}
                <button onClick={() => deleteCar(car.id)} style={{ background: 'transparent', color: '#6B7280', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, padding: '6px 12px', ...FB, fontSize: 12, cursor: 'pointer' }}>
                  Delete
                </button>
              </div>
            </div>
