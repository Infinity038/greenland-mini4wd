...
export default function AdminRaceResults() {
    ...
    async function loadData() {
        ...
        setResults(r.data || []);
        if (cfg.data)
            setMinRaces(parseInt(cfg.data.value));
        const active = s.data?.find((x: any) => x.is_active);
        if (active)
            setForm(f => ({ ...f, season_id: active.id }));
    }
    ...
    async function handleSubmit() {
        if (!form.season_id || !form.race_category || !form.member_id || !form.race_date) {
            setMsg('Fill in season, category, member, and race date.');
            return;
        }
        setSaving(true);
        ...
        if (!error) {
            try {
                ...
                if (lifetimeRaces > 0) {
                    ...
                    if (r.broke_record)
                        hofMsg += ' 🏆 New Most Races record!';
                }
            }
            catch (e: any) {
                hofMsg = ' ⚠️ HOF check failed: ' + e.message;
            }
            try {
                await syncSeasonTiers(form.season_id);
            }
            catch (e: any) {
                hofMsg += ' ⚠️ Tier sync failed: ' + e.message;
            }
        }
        if (error) {
    ...
    async function handleDelete(id: string) {
        ...
        if (row?.season_id) {
            try {
                await syncSeasonTiers(row.season_id);
            }
            catch { /* non-fatal */ }
        }
        loadData();
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={s.card}>
          ...
          <select style={s.select} value={form.selected_tournament} onChange={e => handleTournamentSelect(e.target.value)}>
            ...
            {tournaments.map((t: any) => (<option key={t.id} value={t.id}>{t.name}{t.date ? ` — ${new Date(t.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}` : ''}</option>))}
          ...
          {form.selected_tournament === '__custom__' ? (<>
            ...
            </>) : form.selected_tournament && (<div style={s.hint}>Date: {form.race_date ? new Date(form.race_date + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}</div>)}
