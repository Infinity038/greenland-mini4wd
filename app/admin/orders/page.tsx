...
function ProductThumb({ url }: {
    ...
    if (!first || failed)
        return (<div style={{ width: 56, height: 56, borderRadius: 8, background: '#050505', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
export default function AdminOrders() {
    ...
    function extractSpendFromNotes(notes: string): number {
        ...
        if (full)
            return parseFloat(full[1]);
        const deposit = notes.match(/Deposit:\s*([\d.]+)\s*DKK\s*\(Remaining:\s*([\d.]+)\s*DKK/i);
        if (deposit)
            return parseFloat(deposit[1]) + parseFloat(deposit[2]);
    ...
    return (<div style={s.page}>
      ...
      <div style={s.body}>
        ...
        {tab === 'orders' && (<>
            <div style={s.filters}>
              {['all', ...STATUSES].map(f => (<button key={f} style={s.filterBtn(filter === f)} onClick={() => setFilter(f)}>
                  {f === 'all' ? 'ALL' : STATUS_LABELS[f]?.toUpperCase()}
                </button>))}
            </div>

            {filtered.length === 0 ? (<div style={{ textAlign: 'center', padding: '60px 20px', color: '#4B5563' }}>No orders found.</div>) : filtered.map((order: any) => {
                const imgUrl = getProductImage(order.product_name);
                return (<div key={order.id} style={s.card}>
                  ...
                  {order.proof_url && (<div style={s.proof}>
                      <a href={order.proof_url} target="_blank" rel="noreferrer" style={s.proofLink}>📸 View Payment Proof</a>
                    </div>)}

                  {order.admin_notes && (<div style={{ fontSize: '12px', color: '#6B7280', marginTop: '8px' }}>Note: {order.admin_notes}</div>)}

                  <div style={s.actions}>
                    <select style={s.select} value={notes[`${order.id}_status`] || order.payment_status} onChange={e => setNotes(n => ({ ...n, [`${order.id}_status`]: e.target.value }))}>
                    ...
                    <button style={s.saveBtn} disabled={saving === order.id} onClick={() => updateStatus(order.id, notes[`${order.id}_status`] || order.payment_status)}>
        ...
        {tab === 'members' && (<div style={{ background: '#071426', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '10px', padding: '20px' }}>
            {members.length === 0 ? (<div style={{ textAlign: 'center', padding: '40px', color: '#4B5563' }}>No members yet.</div>) : members.map((m: any) => {
                const fullName = m.name || `${m.first_name || ''} ${m.last_name || ''}`.trim();
                return (<div key={m.id} style={s.memberRow}>
                <div>
