...
export default function AdminNewsPage() {
    const [posts, setPosts] = useState<any[]>(DEMO);
    const [editing, setEditing] = useState<any>(null);
    const [saving, setSaving] = useState(false);
    const [useDb, setUseDb] = useState(false);
    useEffect(() => { fetchPosts(); }, []);
    const fetchPosts = async () => { const { data, error } = await supabase.from('news_posts').select('*').order('created_at', { ascending: false }); if (!error && data) {
        setPosts(data);
        setUseDb(true);
    } };
    const save = async () => {
        ...
        if (useDb) {
            ...
            if (editing.id)
                ...
                setPosts(p => [{ ...editing, id: Date.now().toString(), created_at: new Date().toISOString() }, ...p]);
        }
        setEditing(null);
        setSaving(false);
    };
    ...
    const toggle = async (post: any) => { const u = { ...post, published: !post.published }; if (useDb) {
        await supabase.from('news_posts').update({ published: u.published }).eq('id', post.id);
        await fetchPosts();
    }
    else
        setPosts(p => p.map(x => x.id === post.id ? u : x)); };
    return (<div style={{ minHeight: '100vh', background: '#050505', color: '#F5F5F5' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {posts.map(post => (<div key={post.id} style={{ background: '#071426', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '16px 20px' }}>
      ...
      {editing && (<div onClick={() => setEditing(null)} style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#071426', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 18, width: '100%', maxWidth: 580, marginBottom: 24 }}>
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
              {editing.tournament_id && (<div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8, padding: '10px 14px', ...FB, fontSize: 12, color: '#FCA5A5' }}>
